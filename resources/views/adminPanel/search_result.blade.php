...
<a href="{{url($url)}}">
  <button class="btn btn-warning btn-lg"><i class="fas fa-arrow-left"></i>    GO BACK</button>
  </a>
  <br><br>
<div class="row">
    <div class="col-sm-9">
...
<tbody id="myTable">
    ...
    <tr>
       ...
       <?php $va = $values->$key; ?>
       @if($key == "status")
       <?php $va = $values->status->status; ?>
       @endif
       @if($key == "organization")
       <?php $va = $values->organization->name; ?>
       @endif
       @if($key == "package")
       <?php $va = $values->package->name; ?>
       @endif
       @if($key == "user")
       <?php $va = $values->user->email; ?>
       @endif
       @if($key == "library")
       <?php $va = $values->library->file_name; ?>
       @endif
       @if($key == "type")
       <?php $va = $values->type->name; ?>
       @endif
       @if($key == "subject")
       <?php $va = $values->subject->name; ?>
       @endif
       @if($key == "level")
       <?php $va = $values->level->name; ?>
       @endif
       @if($key == "class")
       <?php $va = $values->class->name; ?>
       @endif
       @if($key == "status_name")
       <?php $va = $values->status ?>
       @endif
    ...
    <td >{{$va}}</td>
       ...
       <td>   
           ...
           @if(!isset($disable))
              <!--DELETE THE ENTRY-->
              <form action="{{ $url.'/'.$values->id }}" method="POST" onsubmit="confirm('Are you sure, You want to delete?')">
              {{csrf_field()}}
